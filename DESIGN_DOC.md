# Design Document: MUSE-QD Implementation

## System Architecture
The system follows a hybrid architecture to balance performance with ease of experimentation.

Rust Backend (muse_rs): Handles the "Survival Step." It manages the unstructured archive, computes Euclidean distances, and executes the Greedy Subset Selection loop. This is critical because the selection logic is $O(K^2)$ or $O(K \log K)$ and requires heavy array operations.
...
Python Frontend (muse_qd): Manages the evolutionary loop (generation of offspring), interacts with the simulation (fitness/descriptor evaluation), and handles data logging/visualization.
...
*   Implement Individual and MuseArchive structs.
...
*   Generate heatmaps (using PCA/t-SNE for high-dim visualization) and QD-Score plots.## Test Plan: MUSE-QD vs. MAP-Elites
To validate the claims in the document, you must prove that MUSE-QD works where MAP-Elites breaks.

### Test 1: The "Sanity Check" (Low Dimension)
*   **Domain:** 2D or 3D Point Navigation / Standard Arm Reaching.
*   **Configuration:** MAP-Elites (50x50 grid) vs. MUSE-QD ($K=2500$).
*   **Goal:** Confirm MUSE-QD achieves similar coverage and max fitness to standard methods in easy domains.
...
*   **Comparison:**
    ...
    *   MUSE-QD: Fixed $K=1000$ or $K=5000$.
*   **Metric:** "QD-Score" (Sum of fitness of all elites) and "Coverage" (Percentage of reachable workspace covered).
*   **Hypothesis:** MUSE-QD will form a diverse cloud of solutions; MAP-Elites will fail to expand.

### Test 3: Stability Comparison (vs MOUR-QD)
*   **Domain:** Unbounded continuous optimization (e.g., Rastrigin function).
*   **Configuration:** Run MOUR-QD with varying radii ($l$) and MUSE-QD with fixed $K$.
*   **Goal:** Demonstrate that MOUR-QD's archive size fluctuates unpredictably (explodes or collapses) depending on the radius, whereas MUSE-QD maintains exactly $K$ elites, satisfying the "Fixed Memory" constraint.
