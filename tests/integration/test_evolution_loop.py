...
class TestEvolutionLoop:
    ...
    def test_reproducibility(self, arm_task):
        ...
        result1 = run_mmr_elites(
            task=arm_task, archive_size=50, generations=20,
            batch_size=20, lambda_val=0.5, mutation_sigma=0.1, seed=42,
        )
        ...
        np.testing.assert_array_equal(
            result1["final_fitness"],
            result2["final_fitness"]
        )
...
class TestAlgorithmComparison:
    ...
    def test_mmr_elites_better_uniformity(self, arm_task):
        ...
        mmr_result = run_mmr_elites(
            task=arm_task, archive_size=100, generations=200,
            batch_size=50, lambda_val=0.5, mutation_sigma=0.1, seed=42,
        )
        
        me_result = run_map_elites(
            task=arm_task, generations=200, batch_size=50,
            bins_per_dim=5, mutation_sigma=0.1, seed=42,
        )
