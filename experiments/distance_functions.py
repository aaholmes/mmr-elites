...
def normalized_euclidean(b1: np.ndarray, b2: np.ndarray, max_dist: Optional[float] = None) -> float:
...
def get_distance_function(
    ...
    if name == 'euclidean':
        ...
        if sigma is None:
            raise ValueError("sigma required for gaussian distance (or use 'gaussian_auto')")
        return partial(gaussian_saturating, sigma=sigma)
    
    elif name == 'exponential':
        if sigma is None:
            raise ValueError("sigma required for exponential distance (or use 'exponential_auto')")
