...
def compute_metrics(
    ...
    n = len(fitness)
    if n == 0:
        return {k: 0.0 for k in ["qd_score", "qd_score_at_budget", "max_fitness",
                                  "mean_fitness", "mean_pairwise_distance", 
                                  "uniformity_cv", "archive_size", "coverage_efficiency"]}
...
class ArmTask:
    ...
    def __init__(self, n_dof: int = 20, target_pos: Tuple[float, float] = (0.8, 0.0),
...
def run_mmr_elites(
    ...
    history = {k: [] for k in ["generation", "qd_score", "qd_score_at_budget",
                                "max_fitness", "mean_pairwise_distance", 
    ...
    for gen in range(1, config.generations + 1):
        ...
        offspring = np.clip(
            parents + np.random.normal(0, config.mutation_sigma, (config.batch_size, n_dof)),
            -np.pi, np.pi
        )
...
def run_cvt_map_elites(
    ...
    if SKLEARN_AVAILABLE:
        centroids = KMeans(n_clusters=config.archive_size, random_state=seed, 
                          n_init=1, max_iter=50).fit(samples).cluster_centers_
    else:
...
def run_random_search(
    ...
    return {
        ...
        "seed": seed,
        "runtime": time.time() - start,
        "final_metrics": compute_metrics(fit_arr[top_idx], desc_arr[top_idx], config.archive_size),
        "history": history,
...
def run_dimensionality_scaling(output_dir: Path, n_seeds: int = 10, quick: bool = False):
    """
    ...
    results = {d: {"MMR-Elites": [], "MAP-Elites": [], "CVT-MAP-Elites": [], "Random": []}
               for d in dimensions}
    ...
    for d in dimensions:
        ...
        for alg in ["MMR-Elites", "MAP-Elites", "CVT-MAP-Elites", "Random"]:
            if results[d][alg]:
                vals = [r["final_metrics"]["qd_score_at_budget"] for r in results[d][alg]]
...
def run_lambda_ablation(output_dir: Path, n_seeds: int = 5, quick: bool = False):
    ...
    lambda_values = [0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0] if not quick else [0.0, 0.5, 1.0]
    ...
    for lam in lambda_values:
        ...
        print(f"{lam:<8.1f} | {np.mean(qd):>7.1f} ± {np.std(qd):<6.1f} | "
...
def main():
    ...
    parser.add_argument("--experiment", choices=["all", "scaling", "lambda", "archive", "runtime"],
                        default="all", help="Which experiment to run")
    parser.add_argument("--quick", action="store_true", help="Quick test mode")
    parser.add_argument("--seeds", type=int, default=10, help="Number of seeds")
    parser.add_argument("--output-dir", default="results/full_benchmark", help="Output directory")
