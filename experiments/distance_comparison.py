        ...
        for name in results[dim]:
            ...
            for metric, source in metrics:
                if source == "final_metrics":
                    values = [
                        r["final_metrics"].get(metric, 0) for r in results[dim][name]
                    ]
                else:
                    values = [
                        r["coverage_metrics"].get(metric, 0) for r in results[dim][name]
                    ]
...
def run_sigma_sensitivity(
    ...
    for mult in sigma_multipliers:
        ...
        # Summary
        qd_vals = [
            r["final_metrics"]["qd_score_at_budget"] for r in results[mult]["runs"]
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sigma-test", action="store_true", help="Run sigma sensitivity analysis only"
    )
    parser.add_argument(
        "--dimensions", type=int, nargs="+", default=None, help="Dimensions to test"
    )
    parser.add_argument("--seeds", type=int, default=5, help="Number of seeds")
    parser.add_argument(
        "--generations", type=int, default=500, help="Generations per run"
    )
