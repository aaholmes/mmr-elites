...
class ArmTask(Task):
    ...
    def __init__(
        ...
        obstacle: Optional[Tuple[float, float, float, float]] = (0.5, 0.55, -0.25, 0.25),
    ):
        """
        Initialize arm task.
    ...
    def check_collisions_batch(self, joint_coords: np.ndarray) -> np.ndarray:
        ...
        hit = (check_vertical(x_min) | check_vertical(x_max) |
               check_horizontal(y_min) | check_horizontal(y_max))
