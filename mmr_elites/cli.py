...
@main.command()
@click.option("--task", type=click.Choice(["arm", "rastrigin"]), default="arm",
              help="Benchmark task to run")
@click.option("--algorithm", type=click.Choice(["mmr_elites", "map_elites", "cvt_map_elites", "random"]),
              default="mmr_elites", help="Algorithm to run")
@click.option("--generations", "-g", type=int, default=1000, help="Number of generations")
@click.option("--archive-size", "-k", type=int, default=1000, help="Archive size K")
@click.option("--batch-size", "-b", type=int, default=200, help="Offspring per generation")
@click.option("--lambda-val", "-l", type=float, default=0.5, help="Diversity weight λ (MMR-Elites only)")
@click.option("--n-dof", "-d", type=int, default=20, help="Degrees of freedom (arm task)")
@click.option("--seed", "-s", type=int, default=42, help="Random seed")
@click.option("--output", "-o", type=click.Path(), default=None, help="Output directory")
@click.option("--quiet", "-q", is_flag=True, help="Suppress progress output")
def run(task, algorithm, generations, archive_size, batch_size, lambda_val, n_dof, seed, output, quiet):
    ...
    if not quiet:
        ...
        click.echo(f"   Generations: {generations}, Archive: {archive_size}, Seed: {seed}")
    
    # Run algorithm
    algorithms = {
        "mmr_elites": lambda: run_mmr_elites(
            task_obj, archive_size=archive_size, generations=generations,
            batch_size=batch_size, lambda_val=lambda_val, seed=seed
        ),
        ...
        "cvt_map_elites": lambda: run_cvt_map_elites(
            task_obj, n_niches=archive_size, generations=generations,
    ...
    # Display results
    metrics = result.final_metrics if hasattr(result, 'final_metrics') else result["final_metrics"]
    runtime = result.runtime if hasattr(result, 'runtime') else result["runtime"]
    
    if not quiet:
        click.echo(f"\n📊 Results:")
        click.echo(f"   QD-Score:     {metrics['qd_score']:.2f}")
        click.echo(f"   QD@Budget:    {metrics.get('qd_score_at_budget', metrics['qd_score']):.2f}")
        click.echo(f"   Max Fitness:  {metrics['max_fitness']:.4f}")
        click.echo(f"   Mean Fitness: {metrics['mean_fitness']:.4f}")
        click.echo(f"   Uniformity:   {metrics.get('uniformity_cv', 0):.4f} (lower=better)")
    ...
    if output:
        ...
        summary = {
            "algorithm": result.algorithm if hasattr(result, 'algorithm') else result["algorithm"],
            "seed": seed,
            "runtime": runtime,
...
@click.option("--full", is_flag=True, help="Full benchmark (10 seeds, 2000 generations)")
...
@click.option("--output", "-o", type=click.Path(), default="results/benchmark",
...
def benchmark(quick, full, seeds, generations, output):
    ...
    algorithms = {
        ...
        "CVT-MAP-Elites": lambda s: run_cvt_map_elites(task, generations=generations, seed=s),
    ...
    for seed in range(seeds):
        ...
        for name, alg_fn in algorithms.items():
            ...
            try:
                ...
                metrics = r.final_metrics if hasattr(r, 'final_metrics') else r["final_metrics"]
    ...
    for name, runs in results.items():
        if runs:
            ...
            click.echo(f"{name:20s}: {np.mean(qd_scores):8.2f} ± {np.std(qd_scores):.2f}")
...
@main.command()
@click.option("--dimensions", "-d", multiple=True, type=int, default=[5, 10, 20, 50],
              help="Dimensions to test (can specify multiple)")
@click.option("--seeds", "-n", type=int, default=5, help="Seeds per dimension")
@click.option("--generations", "-g", type=int, default=1000, help="Generations per run")
@click.option("--output", "-o", type=click.Path(), default="results/scaling",
              help="Output directory")
